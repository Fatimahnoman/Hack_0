    ...
    if yaml_data['type'] == 'email':
        ...
        if has_important_keywords(content):
            ...
            if new_name:
                ...
                # Step 2: Auto-generate reply draft and move to Plans
                needs_action_path = NEEDS_ACTION_FOLDER / new_name
                reply_draft = generate_reply_draft(yaml_data, content)
                
                # Add reply draft to the file
                file_content = needs_action_path.read_text(encoding='utf-8')
                file_content += f"\n\n{reply_draft}"
                needs_action_path.write_text(file_content, encoding='utf-8')
                
                # Move to Plans
                plans_name = move_file(needs_action_path, PLANS_FOLDER, "")
                if plans_name:
                    logger.info(f"  → Reply generated and moved to Plans: {plans_name}")
                    
                    # Step 3: Auto-move to Pending_Approval
                    plans_path = PLANS_FOLDER / plans_name
                    pending_name = move_file(plans_path, PENDING_APPROVAL_FOLDER, "")
                    if pending_name:
                        logger.info(f"  → Auto-moved to Pending_Approval: {pending_name}")
