...
class InboxEventHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        if original_name.endswith('.md'):
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Skipping .md file (Gmail watcher handles): {original_name}")
            return

        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] New file detected: {original_name}")
