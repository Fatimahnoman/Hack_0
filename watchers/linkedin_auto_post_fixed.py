...
==============================================
Direct copy of working tools/linkedin_auto_poster.py
Just run and post approved files automatically.
"""
...
APPROVED_FOLDER = PENDING_FOLDER / "Approved"
DONE_FOLDER = PROJECT_ROOT / "Done"
LOGS_FOLDER = PROJECT_ROOT / "Logs"
SESSION_PATH = PROJECT_ROOT / "session" / "linkedin"
DEBUG_FOLDER = PROJECT_ROOT / "debug_linkedin"

for d in [PENDING_FOLDER, APPROVED_FOLDER, DONE_FOLDER, LOGS_FOLDER, SESSION_PATH, DEBUG_FOLDER]:
    d.mkdir(parents=True, exist_ok=True)

# Setup logging
log_file = LOGS_FOLDER / f"linkedin_auto_{datetime.now().strftime('%Y%m%d')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def save_debug(page, name):
    ...
    try:
        ...
        logger.info(f"Debug saved: {name}")
...
def wait_for_login(page, timeout: int = 180) -> bool:
    ...
    while time.time() - start < timeout:
        ...
        logger.info(f"Waiting... ({elapsed}/{timeout}s)")
        time.sleep(5)
...
def post_to_linkedin(content: str) -> bool:
    """
    Post content to LinkedIn - WORKING VERSION.
    
    This is the exact code from tools/linkedin_auto_poster.py that was working before.
    """
    logger.info("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Click modal backdrop for focus
            try:
...
def main():
    """Main function - post all approved drafts."""
    print("=" * 60)
    print("LinkedIn Auto Poster - SIMPLE WORKING VERSION")
    print("=" * 60)
    print(f"Session: {SESSION_PATH}")
    print(f"Approved: {APPROVED_FOLDER}")
    print(f"Log: {log_file}")
    print("-" * 60)
    print("This will auto-type and auto-submit posts!")
    print("=" * 60)
    ...
    if not approved:
        ...
        print(f"Move drafts to: {APPROVED_FOLDER}")
        return
