...
# Scopes: readonly (read emails) + send (send replies) + modify (mark as read)
SCOPES = [
...
sys.path.insert(0, str(PROJECT_ROOT / "Bronze_Tier"))
try:
    from dashboard_updater import update_dashboard, email_received
except ImportError:
    ...
    update_dashboard = None
    email_received = None
    print("[WARNING] Dashboard updater disabled due to encoding issue")
...
def mark_as_read(service, msg_id: str):
    ...
    try:
        ...
        logger.info(f"✓ Email marked as READ: {msg_id}")
        return True
    except Exception as e:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            for msg in messages:
                ...
                # Update dashboard - email received
                if email_received and update_dashboard:
                    try:
                        email_received(email_data['subject'])
                    except UnicodeEncodeError:
                        logger.debug("Dashboard update skipped (encoding issue)")
                
                # STEP 1: Mark email as READ FIRST (before any checks)
                logger.info(f"Marking email as READ: {email_data['subject']}")
                mark_as_read(service, msg_id)
                
                # STEP 2: Check for important keywords
                ...
                if any(keyword in text_to_check for keyword in IMPORTANT_KEYWORDS):
                    # STEP 3: Create .md file in Needs_Action
                    filename = create_needs_action_file(email_data, msg_id)
                    
                    # Update dashboard - file created
                    if update_dashboard:
                        try:
                            update_dashboard(event="Gmail File Created", folder="Needs_Action/", details=filename)
                        except UnicodeEncodeError:
                            logger.debug("Dashboard update skipped (encoding issue)")
                    ...
                    logger.info(f"Email read but no important keywords: {email_data['subject']}")
                    print(f"  -> Read (no keywords): {email_data['subject']}")
