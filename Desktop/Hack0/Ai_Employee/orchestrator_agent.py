...
try:
    from Bronze_Tier.dashboard_updater import update_dashboard, reply_sent
except ImportError:
...
def process_approved_file(filepath: Path) -> bool:
    ...
    if reply_sent:
        ...
        # Update dashboard - reply sent
        if update_dashboard:
            update_dashboard(event="📧 Reply Sent", folder="Done/", details=f"Reply to {email_data['from']}")

        # Add completion info to the SAME file
...
"""
        ...
        # Move SAME file to Done (only if reply was sent)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        logger.warning(f"  ⚠️  File LEFT in Approved/ (reply not sent)")
        
        # Update dashboard - reply failed
        if update_dashboard:
            update_dashboard(event="❌ Reply Failed", folder="Approved/", details="Credentials issue")
...
"""
        ...
        # DO NOT move to Done - leave in Approved for retry
        logger.info(f"  ⚠️  File kept in Approved/ for retry")
